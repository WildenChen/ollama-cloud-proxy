...
- 再次遮蔽或關閉彈窗時會清除頁面中的 Cookie 值；顯示期間維持唯讀，避免儲存設定時意外覆寫。
- Cookie 顯示端點限制管理員存取並禁止快取，既有列表與用量 API 繼續遮蔽敏感資料。
- 改善手機與平板彈窗配置，眼睛按鈕保留 44 × 44 像素觸控範圍。
- 新增驗證授權、禁止快取、遮蔽清除、彈窗關閉及敏感資料不外洩的回歸測試。
