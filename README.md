...
如果你是第一次安裝，也可以先保留 `ADMIN_TOKEN`。它只在第一次建立管理密碼時使用：
...
```env
ADMIN_TOKEN=my-first-admin-password-please-change
```
...
1. 畫面會明確顯示首次設定狀態；如果 `.env` 有設定 `ADMIN_TOKEN`，在首次設定欄位輸入它。
2. 建立日後登入使用的管理密碼。完成後 `ADMIN_TOKEN` 不再用於登入。
...
- 連 `/admin` 平時只用管理密碼；`ADMIN_TOKEN` 只供第一次建立管理密碼。
